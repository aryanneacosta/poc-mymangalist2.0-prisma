...
async function updateNewVolumeRead(req: Request, res: Response) {
    ...
    const verifyVolumeId = (await findVolume(verifySerieId, volumeExist)).id;
    if(!verifyVolumeId) {
...
async function deletedVolume(req: Request, res: Response) {
    ...
    const volumeDeleted = (await deleteVolume(Number(volumeId))).id;
    if (volumeDeleted === null) {
